...
for (my $i = 0; $i <= $num_samples; $i++) {
    print OUTFILE ",avg_depth_inc_",$sample_names[$i],",avg_depth_all_",$sample_names[$i],",S_",$sample_names[$i],",pi_",$sample_names[$i];
    print OUTFILE ",TajD_",$sample_names[$i],",TajDscaled_",$sample_names[$i];
}
for (my $i = 0; $i < $num_samples; $i++){
        ...
        if ($PassFail eq 'PASS'){
            ...
            if ($PassFail eq 'PASS'){
                ...
                if ($window_count == $window_size) {
                    ...
                    #   the number of depths retained.  "TajD_scaled" is TajD scaled by the abs(minimum possible Tajima's D value).                     #
                    #   (assumes that all SNPs are singletons).                                                                                         #
                    #                                                                                                                                   #
                    #####################################################################################################################################
                    ...
                    for (my $cur_sample = 0; $cur_sample <= $num_samples; $cur_sample++){
                        ...
                        #temporarily set TajD and TajD_scaled to zero for calculation purposes
                        
                        $TajD[$cur_sample] = 0;
                        $TajD_scaled[$cur_sample] = 0;
                        
                        my $num_depths_used = 0;
                        # Calculate a TajD for each sample depth (greater than min_depth), as long as there are at least 4 observations at that depth
                        for (my $cur_depth = $min_TajD_depth; $cur_depth < $max_depth; $cur_depth++) {
                            if ($S_tmp[$cur_sample * $max_depth + $cur_depth] > 3){
                                ...
                                #also calculate the TajD scaled by its theoretical min, assumine all $local_S sites are singletons
                                my $kmin = $local_S * 2 * ($cur_depth - 1) / ($cur_depth * $cur_depth);
                                $TajD_scaled[$cur_sample] += ($local_pi - ($local_S / $a1[$cur_depth])) /(abs ($kmin - ($local_S / $a1[$cur_depth])));
                        ...
                        if ($num_depths_used > 0){
                            ...
                            $TajD_scaled[$cur_sample] = $TajD_scaled[$cur_sample] /$num_depths_used;
                        }
                        else{
                            $TajD[$cur_sample] = 'NA';
                            $TajD_scaled[$cur_sample] = 'NA';
                        }
                    ...
                    for (my $i = 0; $i <= $num_samples; $i++) {
                        print OUTFILE ",",$mean_depth_included[$i],",",$mean_depth_all[$i],",",$final_S_count[$i],",",$final_pi[$i],",",$TajD[$i],",",$TajD_scaled[$i];
                    }
                    for (my $i = 0; $i < $num_samples; $i++){
                        for (my $j = $i + 1; $j < $num_samples; $j++){
