...
###SD-ZI125 as reference (arbitrary)

makebin = function(x) {ifelse(x==snps$A2.C7.Z1125,0,1)}

#bin$A1.Z1348 = makebin(bin$A1.Z1348)

bin = select(snps,-MELREF, -CHROM,-num_A,-num_T,-num_G,-num_C, pos = POS)
#bin = select(snps, -CHROM,-num_A,-num_T,-num_G,-num_C, pos = POS)
...
order_tree <- read_delim("~/Documentos/investigacion/ur_larracuente_presgraves/SD_SNPs/recmin/order_tree.txt",
...
notbin = select(snps,-MELREF, -CHROM,-num_A,-num_T,-num_G,-num_C, pos = POS)
mdat = gather(notbin,'ind','nucl',-pos)
head(hdat)
str(hdat)
mdat$ind = sub('.*\\.', '', hdat$ind)
mdat$ind = sub('Z1',"ZI",hdat$ind)

mdat2 = left_join(mdat,order_tree,'ind') %>%
  arrange(desc(phylo)) %>%
  mutate(ind2 = factor(ind, levels=order_tree$ind))



hplot_col = ggplot(mdat2,aes(x = pos,y = ind2,color=nucl,fill=nucl))+
  geom_point(size=1.5)+
  xlim(c(min(in2rmal),max(in2rmal)))+
  xlab("2R")+
  ylab("SD")+
...
#loc <- read.csv("~/Documentos/investigacion/ur_larracuente_presgraves/SD_SNPs/recmin/recmin_data/locations_table", header=FALSE)
loc <- read.csv("~/Documentos/investigacion/ur_larracuente_presgraves/SD_SNPs/recmin/RecMin/bin_data_refZI125.out.locs.txt",header = FALSE)
str(loc)
...
nobin_hapl = plot_grid(hplot_col,rm,nrow = 2,align = 'v',rel_heights = c(8,2))
nobin_hapl
