...
outF = open("gof_sim_%d_%f_%s.txt" % (obs_s,tsweep,out) , "w")
outF.write("# Nsims= %d ; S = %d ; tsweep = %f \n#pi s tajD\n" % (nreps,obs_s,tsweep))
outF.close()
...
os.system("./ms %d %d -s %d -eN %f %f | ./sample_stats |  awk '{print $2,$4,$6}' >> gof_sim_%d_%f_%s.txt" % (nsam, nreps, s, t, x1,obs_s,tsweep,out))
